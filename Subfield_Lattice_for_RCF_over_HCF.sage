...
def HCF_and_RCFconstructor(a, b):
	...
	# Eli: optimizing the HCF representation because this should speed up constructing the RCF at least somewhat
	HCF_temp = K.extension(hilbert_polynomial, 'temp_z').absolute_field('z')
	HCF, from_HCFopt, to_HCFopt = HCF_temp.optimized_representation(both_maps = True)
	hilbert_polynomial = hilbert_polynomial.change_ring(HCF_temp)
	chosen_j_invariant = hilbert_polynomial.roots()[-1][0]
	chosen_j_opt = to_HCFopt(chosen_j_invariant)
	E=EllipticCurve(j=HCF(chosen_j_opt))
...
### Eli: Adding function to check directly whether the primitive generator of the RCF over the HCF above is part of a normal basis
##		 Inputs: a, b - as above
##		 Outputs: boolean, True if the generator does give a normal basis, false otherwise
def check_normal_basis(a,b):
	# Get RCF, HCF data from Joe's code
	HCF, RCF = HCF_and_RCFconstructor(a,b)[:2]

	# Get absolute RCF to play nicely with the Galois group code
	RCF_abs.<absgen> = RCF.absolute_field()
	from_RCFabs, to_RCFabs = RCF_abs.structure()

	# Find the Galois group of R over H
	R.<x> = QQ[]
	K = QuadraticField(-a)
	HCP = K.hilbert_class_polynomial()
	for grp in RCF_abs.galois_group().subgroups():
		int_fld = grp.fixed_field()[0]
		if len((x^2 + a).change_ring(int_fld).roots()) == 2 and len(HCP.change_ring(int_fld).roots()) == HCP.degree() and grp.cardinality() == RCF.relative_degree():
			GalRoverH = grp
			break

	# Get conjugates of the generator
	conjugates = [g(to_RCFabs(RCF.gens()[0])) for g in GalRoverH]

	# Check for linear independence
	V, from_vs, to_vs = RCF_abs.vector_space()
	vs_conjs = [to_vs(conj) for conj in conjugates]
	# Just a sanity check
	assert(len(vs_conjs) > 0)
	# if there is only one vector in the above list, Sage will get confused, so we put it in a list
	if len(vs_conjs) == 1:
		vs_conjs = [vs_conjs]
	if len(V.linear_dependence(vs_conjs)) == 0:
		return True
	else:
		return False
