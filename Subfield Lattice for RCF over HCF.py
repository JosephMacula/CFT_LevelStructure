...
def HCF_and_RCFconstructor(a, b):
	...
	return(HCF, RCF, divpoly.factor()[-1][0])
...
#Get the Galois group of RCF over HCF

def Gal_RoverH(RCF, HCF):

	G = RCF.absolute_field(names='t').galois_group()
	for i in G.subgroups():
		subfield = i.fixed_field()[0]
		if HCF.is_isomorphic(subfield) == True:
			return i

#An example with K = Q(sqrt(-7)) and modulus (3). As Eli showed, this is an example where we do not have the normal basis we hope for. 
#Nevertheless, the unique intermediate field between HCF and RCF is still generated by a trace sum, as the code below demonstrates. 

HCF, RCF, RCFminpoly = HCF_and_RCFconstructor(7,3)
intermediate_fields = HCF_to_RCF_fields(RCF, HCF)
G = Gal_RoverH(RCF, HCF)
alpha = Gal_RoverH[0](RCF.gens()[0])
g = Gal_RoverH.gens()[1] #somewhat annoyingly, .gens() does not return a minimal set of generators. Checked manually that this element generates the group
g2 = g*g
f = (alpha + g2(alpha)).minpoly()
int_fld.<t> = HCF.extension(f)
int_fld.is_isomorphic(intermediate_fields[1])
